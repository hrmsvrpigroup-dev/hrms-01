...
function StatCard({ icon, label, value, sub, color }: { icon: React.ReactNode; label: string; value: string | number; sub?: string; color: string }) {
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: '20px 24px', border: '1px solid #e8ecf4', display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      ...
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ width: '100%', height: 80, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 2 }}>
            <div style={{ width: '60%', height: `${(d.total / max) * 100}%`, background: '#e0e7ff', borderRadius: '4px 4px 0 0', minHeight: 4 }} />
            <div style={{ width: '60%', height: `${(d.present / max) * 100}%`, background: 'linear-gradient(180deg,#818cf8,#6366f1)', borderRadius: '4px 4px 0 0', minHeight: d.present > 0 ? 4 : 0 }} />
          </div>
...
function TaskBadge({ status }: { status: string }) {
  const c: any = { DONE: { bg: '#dcfce7', color: '#16a34a', label: 'Done' }, IN_PROGRESS: { bg: '#dbeafe', color: '#2563eb', label: 'In Progress' }, PENDING: { bg: '#fef9c3', color: '#ca8a04', label: 'Pending' } }
  const s = c[status] || { bg: '#f1f5f9', color: '#64748b', label: status }
...
export default function HREmployeePortfolio() {
  ...
  if (!data) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '80vh' }}><div style={{ color: '#64748b' }}>Portfolio not found.</div></div>
    ...
    <>
      ...
      <div className="portfolio-page" style={{ background: '#f8fafc', minHeight: '100vh', padding: '24px 32px' }}>
        <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          <button onClick={() => navigate(-1)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, fontWeight: 600, color: '#374151', cursor: 'pointer', fontSize: 14 }}>
            <ArrowLeft size={16} /> Back
          </button>
          <button onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none', borderRadius: 10, color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 14, boxShadow: '0 4px 14px rgba(99,102,241,0.35)' }}>
        ...
        <div ref={portfolioRef} style={{ maxWidth: 1100, margin: '0 auto' }}>
          {/* Hero */}
          <div className="portfolio-card" style={{ background: 'linear-gradient(135deg,#1e1b4b 0%,#312e81 40%,#4338ca 100%)', borderRadius: 24, padding: '36px 40px', display: 'flex', alignItems: 'center', gap: 32, marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: -60, right: -60, width: 250, height: 250, borderRadius: '50%', background: 'rgba(255,255,255,0.05)' }} />
            <div style={{ width: 100, height: 100, borderRadius: '50%', background: 'linear-gradient(135deg,#818cf8,#a78bfa)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, fontWeight: 900, color: '#fff', flexShrink: 0, boxShadow: '0 8px 32px rgba(0,0,0,0.3)', border: '3px solid rgba(255,255,255,0.2)', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', marginTop: 4, fontWeight: 500 }}>{emp.designation || 'Employee'} {emp.department ? `· ${emp.department}` : ''}</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, marginTop: 16 }}>
                {emp.email && <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.7)', fontSize: 13 }}><Mail size={13} /> {emp.email}</div>}
                {emp.phone && <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.7)', fontSize: 13 }}><Phone size={13} /> {emp.phone}</div>}
                {emp.branch && <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.7)', fontSize: 13 }}><MapPin size={13} /> {emp.branch}</div>}
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.7)', fontSize: 13 }}><Calendar size={13} /> Joined {joiningDate}</div>
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
                <span style={{ background: 'rgba(255,255,255,0.15)', color: '#e0e7ff', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{emp.employeeCode}</span>
                <span style={{ background: emp.status === 'ACTIVE' ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)', color: emp.status === 'ACTIVE' ? '#4ade80' : '#f87171', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{emp.status}</span>
                <span style={{ background: 'rgba(255,255,255,0.1)', color: '#c7d2fe', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{emp.employmentType?.replace('_', ' ')}</span>
              </div>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(10px)', borderRadius: 20, padding: '24px 32px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }}>
              ...
              <div style={{ color: '#c7d2fe', fontSize: 12, fontWeight: 700, marginTop: 10, textTransform: 'uppercase', letterSpacing: 1 }}>Performance</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
            <div className="portfolio-card" style={{ background: '#fff', borderRadius: 20, padding: 28, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                <div style={{ background: '#e0e7ff', padding: 8, borderRadius: 10, color: '#6366f1' }}><Activity size={18} /></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: item.bg, borderRadius: 10 }}>
              ...
              <div style={{ marginTop: 16, padding: '12px 16px', background: '#f8fafc', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#6366f1' }}><Coffee size={16} /><span style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>Avg. Work Hours</span></div>
            ...
            <div className="portfolio-card" style={{ background: '#fff', borderRadius: 20, padding: 28, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                <div style={{ background: '#fef3c7', padding: 8, borderRadius: 10, color: '#d97706' }}><Target size={18} /></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  { label: 'Total', value: task.totalTasks, color: '#6366f1', bg: '#e0e7ff' },
          ...
          {/* Monthly Chart */}
          <div className="portfolio-card" style={{ background: '#fff', borderRadius: 20, padding: 28, marginBottom: 24, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <div style={{ background: '#e0e7ff', padding: 8, borderRadius: 10, color: '#6366f1' }}><BarChart2 size={18} /></div>
          ...
          {task.recentTasks?.length > 0 && (
            <div className="portfolio-card" style={{ background: '#fff', borderRadius: 20, padding: 28, marginBottom: 24, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <div style={{ background: '#fef3c7', padding: 8, borderRadius: 10, color: '#d97706' }}><FileText size={18} /></div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {task.recentTasks.map((t: any, i: number) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#f8fafc', borderRadius: 12, border: isOverdue ? '1px solid #fecdd3' : '1px solid transparent' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: t.status === 'DONE' ? '#16a34a' : t.status === 'IN_PROGRESS' ? '#2563eb' : '#d97706' }} />
                        <span style={{ fontWeight: 600, fontSize: 13, color: '#0f172a' }}>{t.title}</span>
                        {isOverdue && <span style={{ fontSize: 10, fontWeight: 700, color: '#dc2626', background: '#fee2e2', padding: '2px 8px', borderRadius: 20 }}>OVERDUE</span>}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        {t.dueDate && <span style={{ fontSize: 12, color: '#94a3b8' }}>Due: {new Date(t.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' })}</span>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
            <div className="portfolio-card" style={{ background: '#fff', borderRadius: 20, padding: 28, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <div style={{ background: '#ede9fe', padding: 8, borderRadius: 10, color: '#7c3aed' }}><Calendar size={18} /></div>
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < 4 ? '1px solid #f1f5f9' : 'none' }}>
                  <div>
                    ...
                    <span style={{ fontSize: 12, color: '#94a3b8', marginLeft: 8 }}>
                      {new Date(l.fromDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} → {new Date(l.toDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </span>
                  </div>
                  <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: l.status === 'APPROVED' ? '#dcfce7' : l.status === 'PENDING' ? '#fef3c7' : '#fee2e2', color: l.status === 'APPROVED' ? '#16a34a' : l.status === 'PENDING' ? '#d97706' : '#dc2626' }}>{l.status}</span>
                </div>
              ))}
              {leave.leaveBreakdown?.length === 0 && <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: 20 }}>No leave records found.</div>}
            </div>
            ...
            <div className="portfolio-card" style={{ background: '#fff', borderRadius: 20, padding: 28, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              ...
              {payrollHistory?.length === 0 ? (
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < payrollHistory.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                  <div>
                    <span style={{ fontWeight: 700, color: '#0f172a', fontSize: 13 }}>{MONTH_NAMES[p.month - 1]} {p.year}</span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: p.status === 'PAID' ? '#dcfce7' : p.status === 'PROCESSED' ? '#dbeafe' : '#f1f5f9', color: p.status === 'PAID' ? '#16a34a' : p.status === 'PROCESSED' ? '#2563eb' : '#64748b' }}>{p.status}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 12, padding: '16px 0', borderTop: '1px solid #e8ecf4' }}>
